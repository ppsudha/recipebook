...
public class RecipeService implements RecipeRepository {
    ...
    public ArrayList<Recipe> getRecipes() {
        Collection<Recipe> bookCollection = recipeBook.values();
        ArrayList<Recipe> recipes = new ArrayList<>(bookCollection);
        return recipes;
    ...
    public Recipe addRecipe(Recipe recipe) {
        recipe.setrecipeId(uniqueBookId);
        recipeBook.put(uniqueBookId, recipe);
        uniqueBookId += 1;
        return recipe;
    ...
    public Recipe updateRecipe(int recipeId, Recipe recipe) {
        Recipe existingBook = recipeBook.get(recipeId);
        if (existingBook == null) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        }
        if (recipe.getrecipeName() != null) {
            existingBook.setrecipeName(recipe.getrecipeName());
        }
